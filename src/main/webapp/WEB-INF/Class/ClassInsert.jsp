...
<div class="h-[100vh] flex justify-center mt-[100px]">
  <div class="flex flex-col min-w-[50%]">
    <div class="flex flex-row justify-between items-center py-3 my-3 border-b-[2px] border-blue-400">
      <div class="text-2xl text-blue-600 uppercase font-bold">Thêm Lớp</div>
    ...
    <form method="post" name="classInsert" action="class?action=insert">
      <div class="w-[100%] flex flex-col my-6">
        <label for="name" class="text-base font-medium mb-1">Tên lớp: </label>
        <input type="text" name="name" id="name" placeholder="Nhập tên lớp" class="rounded-md px-3 py-2 mb-1 input-required">
      ...
      <div class="w-[100%] flex flex-col my-6">
        ...
        <select id="facultyId" name="facultyId" class="w-full rounded-md px-3 py-2 mb-1">
            <%
                ...
                {
            %>
            <option value="<%= facultyCBB.get(i).getId() %>"><%= facultyCBB.get(i).getName() %></option>
            <%
                }
            %>
        </select>
      ...
      <div class="flex flex-row justify-center mt-10 mb-6">
        <input type="button" onClick="validateForm()" value="Thêm lớp" name="insert" class="text-center m-auto py-2 px-4 min-w-[200px] uppercase text-white bg-blue-500 hover:bg-blue-600 hover:cursor-pointer rounded-md font-semibold">
