        ...
        <div class="py-2 inline-block min-w-full sm:px-6 lg:px-8">
            <div class="overflow-hidden">
                <div class="flex flex-row justify-between items-center py-3 my-3 border-b-[2px] border-blue-400">
                    <div class="text-2xl text-blue-600 uppercase font-bold">Danh sách tài khoản</div>
                    <a href="account?mod=insert" class="py-2 px-3 text-white bg-blue-500 hover:bg-blue-600 rounded-md font-semibold">Thêm tài khoản</a>
                </div>
                <%
                    String searchKeyUsername = (String) request.getParameter("searchKeyUsername");
                    ...
                    if (request.getParameter("statusAccount") != null) {
                        statusAccount = Integer.parseInt(request.getParameter("statusAccount").trim());
                    }
                %>
                <form class="flex flex-row justify-between my-4" method="get" action="account?mod=search">
                    <div class="flex bg-white px-3 py-2 rounded-md w-[300px]">
                        ...
                        <input type="text" name="searchKeyUsername" id="search" placeholder="Nhập Tên tài khoản" class="flex-1 bg-white ml-2 outline-none" value="<%=searchKeyUsername%>" >
                    </div>
                    <div class="flex items-center justify-center">
                        <span class="mr-2 font-normal text-base">Phân quyền: </span>
                        <select class="min-w-[200px] rounded-md px-3 py-2" name="statusAccount">
                            <option value="0" selected>Tất cả</option>
                            <option value="1" <% if(1==statusAccount){ %>selected<% } %>>Quản trị viên</option>
                            <option value="2" <% if(2==statusAccount){ %>selected<% } %>>Thủ thư</option>
                        </select>
                ...
                <table class="min-w-full">
                    ...
                    <% } %>
                        ...
                        <td class="text-sm text-gray-900 font-normal px-6 py-4 whitespace-nowrap text-center">
                            <% if(accountList.get(i).isAdmin()){ %> Quản trị viên 
                            <% } else { %> Thủ thư <% } %>
                        </td>
                        <td class="text-sm text-gray-900 font-normal px-6 py-4 whitespace-nowrap text-center">
                            <ul class="flex justify-evenly text-base">
                                <li>
                                    <a href="?mod=update&id=<%= accountList.get(i).getId() %>">
                                    <i class="view-icon fa-regular fa-pen-to-square"></i>
                                    </a>
                                </li>
                                <li>
                                    <a href="?mod=delete&id=<%= accountList.get(i).getId() %>" class="action-delete">
